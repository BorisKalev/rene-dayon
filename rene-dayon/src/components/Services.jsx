      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {s.items.map(({ title, description }, i) => {
          ...
          return (
            <div
              key={title}
              className="group bg-zinc-950 border border-zinc-800 rounded-lg overflow-hidden hover:border-brand-red transition-colors duration-300"
            >
              {/* Image header */}
              <div className="relative h-40 overflow-hidden">
                <img
                  src={img}
                  alt={title}
                  className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-[filter] duration-500 ease-in-out"
                />
              ...
              {/* Card body */}
              <div className="relative px-6 pb-6 pt-4 overflow-hidden">
                {/* Number */}
                <span
                  aria-hidden="true"
                  className="absolute -top-2 right-4 font-display font-bold leading-none select-none pointer-events-none text-brand-red/30 group-hover:text-brand-red/60 transition-colors duration-300"
                  style={{ fontSize: '4rem' }}
                >
                ...
                <p className="relative text-zinc-500 text-sm leading-relaxed">{description}</p>
              </div>
            </div>
          )
        })}
